...
namespace test_generate {
   ...
   void validate_feedback(const ecfeed::test_arguments& test_arguments) {
      ...
      if (test_arguments.get_string(0) == "a0") {
         test_arguments.get_handle()->add_feedback(false, 100 + (rand() % 200), "Failed - a", {{"key1", "value1"}, {"key2", "value2"}});
         FAIL();
      } else if (test_arguments.get_string(1) == "b1") {
         test_arguments.get_handle()->add_feedback(false, 100 + (rand() % 200), "Failed - b", {{"key1", "value1"}, {"key2", "value2"}});
         FAIL();
      } else if (test_arguments.get_string(7) == "h6") {
         test_arguments.get_handle()->add_feedback(false, 100 + (rand() % 200), "Failed - c", {{"key1", "value1"}, {"key2", "value2"}});
         FAIL();
      }
...
// INSTANTIATE_TEST_CASE_P(gen_random_quantity_single, FixtureGenerate, ::testing::ValuesIn((test_generate::testProvider->generate_random(config::Default::F_10x10, test_generate::gen_random_quantity_single))->to_list()));
// TEST_P(FixtureGenerate, gen_random_quantity_single) {
   
//    test_generate::validate(GetParam());
// }

// INSTANTIATE_TEST_CASE_P(gen_random_quantity_short, FixtureGenerate, ::testing::ValuesIn((test_generate::testProvider->generate_random(config::Default::F_10x10, test_generate::gen_random_quantity_short))->to_list()));
// TEST_P(FixtureGenerate, gen_random_quantity_short) {
   
//    test_generate::validate(GetParam());
// }

// INSTANTIATE_TEST_CASE_P(gen_random_quantity_long, FixtureGenerate, ::testing::ValuesIn((test_generate::testProvider->generate_random(config::Default::F_10x10, test_generate::gen_random_quantity_long))->to_list()));
// TEST_P(FixtureGenerate, gen_random_quantity_long) {
   
//    test_generate::validate(GetParam());
// }

// INSTANTIATE_TEST_CASE_P(gen_random_custom, FixtureGenerate, ::testing::ValuesIn((test_generate::testProvider->generate_random(config::Default::F_10x10, test_generate::gen_random_custom))->to_list()));
// TEST_P(FixtureGenerate, gen_random_custom) {
   
//    test_generate::validate(GetParam());
// }

// INSTANTIATE_TEST_CASE_P(gen_nwise, FixtureGenerate, ::testing::ValuesIn((test_generate::testProvider->generate_nwise(config::Default::F_10x10, test_generate::gen_nwise))->to_list()));
// TEST_P(FixtureGenerate, gen_nwise) {
   
//    test_generate::validate(GetParam());
// }

// INSTANTIATE_TEST_CASE_P(gen_nwise_feedback, FixtureGenerate, ::testing::ValuesIn((test_generate::testProvider->generate_nwise(config::Default::F_10x10, test_generate::gen_nwise_feedback))->to_list()));
